...
#define TLS_PSK_ID		CONFIG_GOLIOTH_LIGHTDB_DTLS_PSK_ID
#define TLS_PSK			CONFIG_GOLIOTH_LIGHTDB_DTLS_PSK
...
static int initialize_client(void)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV4)) {
		...
		addr4->sin_port = htons(CONFIG_GOLIOTH_LIGHTDB_PORT);

		zsock_inet_pton(addr4->sin_family, CONFIG_GOLIOTH_LIGHTDB_IP_ADDR,
				&addr4->sin_addr);
		...
		addr6->sin6_port = htons(CONFIG_GOLIOTH_LIGHTDB_PORT);

		zsock_inet_pton(addr6->sin6_family, CONFIG_GOLIOTH_LIGHTDB_IP_ADDR,
				&addr6->sin6_addr);
